...
int main() {
	...
	cin >> n >> m;
	int ans = n * m;
	multiset<pair<pii,bool>> s;
	for (int i = 0; i < n; i++) {
		for (int j = 0; j < m; j++) {
			char c;
			cin >> c;
			if (c == 'S') s.insert({ { i,j },1 });
		}
	}
	...
	for (auto p : s) {
		...
		for (auto pp : s) {
			if (p == pp or !pp.second) continue;
			if (!s.count({ {p.ff.ff,pp.ff.ss},1 }) and !s.count({ {p.ff.ff,pp.ff.ss},0 })) {
				s.insert({ {p.ff.ff,pp.ff.ss},0 });
			}

			if (!s.count({ {pp.ff.ff,p.ff.ss},1 }) and !s.count({ {pp.ff.ff,p.ff.ss},0 })) {
				s.insert({ {pp.ff.ff,p.ff.ss},0 });
			}
		}
	}
