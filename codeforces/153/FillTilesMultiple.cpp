...
ll solve(ll n, map<int,int> tiles) {
	if (n < 0) return 0;
	if (n == 0) return 1;
	if (dp[n]) return dp[n];
	ll ans = 0;
	for (auto p : tiles) {
		ans += p.ss * solve(n - p.ff, tiles);
	}
	return dp[n] = ans;
}


void FillTilesMultiple() {
	sync;
	// Count the number of ways to fill a n length tile using only 1 block (red, purple - imply two different types of 1 block tiles), 2 block, and 4 block tiles
	ll n,m;
	cin >> n;
	cin >> m;
	map<int, int> tiles;
