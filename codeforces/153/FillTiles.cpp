...
int solve(int n) {
	...
	if (n >= 2) return dp[n] = solve(n - 1) + solve(n - 2);
	else return dp[n] =  solve(n - 1);
}

void FillTiles() {
	sync;
	// Count the number of ways to fill a n length tile using only 1 block and 2 block wide tiles (and the combination of both)
	int n;
	cin >> n;
	dp = vi(n+1, 0);
	cout << solve(n) << '\n';
