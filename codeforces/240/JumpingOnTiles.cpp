...
int main() {
	...
	while (t--) {
		...
		while (f != l) {
			b += m[f].ff;
			ans.insert(ans.end(), m[f].ss.begin(), m[f].ss.end());
			m[f].ff = 0, m[f].ss = {};
			char d = f - 1, g = f+1;
			while (d >= 'a' and !m[d].ff) d--;
			while (g <= 'z' and !m[g].ff) g++;
			if (abs(l - d) < abs(l - g)) {
				a += abs(index(f) - index(d));
				f = d;
			}
			else {
				a += abs(index(f) - index(g));
				f = g;
			}
		}
		b += m[f].ff;
