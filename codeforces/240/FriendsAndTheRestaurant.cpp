...
int main() {
	...
	while (t--) {
		...
		for (int i = 0; i < n; i++) {
			int c = b[i] - a[i];
			if (!c) z++;
			else if (c > 0) p.push_back(c);
			else ne.push_back(abs(c));
		}
		sort(all(p)); sort(all(ne));
		int i = 0, j=0, c =0; 
		int ans = (z / 2);
		while (i < p.size() and j<ne.size()) {
			while (i < p.size() and p[i] < ne[j]) i++;
			if (i < p.size() and p[i] >= ne[j]) ans++,i++,j++,c++;
		}
		if (z % 2 and c < p.size()) ans++,c++;
		ans += (p.size()-c) / 2;
		cout << ans << '\n';
	}
}
