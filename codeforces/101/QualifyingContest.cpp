...
void QualifyingContest() {
	sync;
	int n, m;
	cin >> n >> m;
	vector<vector<pair<int, string>>> c(m+1);
	si d;
	for (int i = 0; i < n; i++) {
		string s;
		int a, b;
	...
	for (int f : d) {
		sort(all(c[f]), greater<pair<int, string>>());
		if (c[f].size() == 2) {
			cout << c[f][0].second << " " << c[f][1].second << '\n';
		}
		else {
			if (c[f][1].first == c[f][2].first) cout << "?" << '\n';
			else cout << c[f][0].second << " " << c[f][1].second << '\n';
		}
	}
}	