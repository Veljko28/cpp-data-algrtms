...
void hasYellow(vi& f,int b,int a, char c, string team) {
	auto t = find(all(f), b);
	if (t != f.end()) {
		bool ok = true;
		for (int i = 0; i < r.size(); i++) {
			if (r[i].team == c and r[i].number == b) {
				ok = false;
				break;
			}
		}
...
void VasyaAndFootball() {
	...
	while (n--) {
		...
		if (d == 'r') {
			...
			if (c == 'h') hasYellow(f, b, a, c, ht);
			else hasYellow(g, b, a, c, at);
		}
	}
}	