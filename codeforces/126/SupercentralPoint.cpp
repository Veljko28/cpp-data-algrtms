...
void SupercentralPoint() {
	...
	for (int i =0;i<n;i++){
		...
		for (int j = 0; j < n; j++) {
			if (j == i) continue;
			if (a[i].ff == a[j].ff and a[i].ss > a[j].ss) b[0] = true;
			if (a[i].ff == a[j].ff and a[i].ss < a[j].ss) b[1] = true;
			if (a[i].ss == a[j].ss and a[i].ff < a[j].ff) b[2] = true;
			if (a[i].ss == a[j].ss and a[i].ff > a[j].ff) b[3] = true;
		}
		if (b[0] and b[1] and b[2] and b[3])  ans++;
	}
	cout << ans;
}