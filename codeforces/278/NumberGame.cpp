...
bool check(vector<int> b, int k, int i, int j = 0) {
	if (k == 0) return true;
	if (i < 0) return false;
	while (i > 0 and b[i] > k) i--;
	if (b[i] > k) return false;
	else {
		b[j] += k;
		return check(b, k - 1, i - 1, j+1);
	}
}
...
int main() {
	...
	while (t--) {
		...
		int ans = 0;
		for (int i = 1; i <= n; i++) {
			auto f = lower_bound(all(a), i);
			if (f == a.end() or (*f > i and f != a.begin())) f--;
			if (check(a, i, f-a.begin())) ans = i;
		}
		cout << ans << '\n';
	}
}
