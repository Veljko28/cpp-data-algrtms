...
void HelpVasilisaTheWise2() {
	...
	vvi ans;
	bool ok = false;
	for (int i = 1; i <= r1-1 and i < 10; i++) {
		int a = r1 - i;
		if (a > 9) continue;
		for (int j = 1; j <= r2-1 and j < 10; j++) {
			int b = r2 - j;
			if (b > 9) continue;
			if (i + b == d1 and a + j == d2 and i + j == c1 and a + b == c2 and (c(i,j) and c(i,a) and c(i,b) and c(j,a) and c(j,b) and c(a,b))) {
				ok = true;
				ans = { {i,a}, {j,b} };
				break;
			}
