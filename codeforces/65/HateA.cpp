...
void make(int i, int j, int n,int m, vector<vi>& ans){
    if (i > 0)
	{
		if (j > 0)
			ans[i - 1][j - 1]++;
...
void HateA() {
   ...
   for (int i = (t.length()/2);i<t.length();i++){
       if (t[i] != t[i-(t.length()/2)]) {
           outn(":(");
           return 0;
       }
   }
   if (s.substr(s.length()-(t.length()/2), s.length()) != t.substr(0,(t.length()/2))){
       outn(":(");
       return 0;
   }
   outn(s.substr(0,s.length()-(t.length()/2)));
}
