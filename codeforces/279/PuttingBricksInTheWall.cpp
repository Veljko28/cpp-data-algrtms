...
int main() {
	...
	while (t--) {
		...
		char u = a[0][1], j = a[1][0], k = a[n - 1][n - 2], l = a[n - 2][n - 1];
		vector<pair<int, int>> ans;
		if (u == j) {
			if (k == u) ans.push_back({ n,n - 1 });
			if (l == u) ans.push_back({ n - 1,n });
		}
		else if (k == l) {
			if (u == k) ans.push_back({ 1,2 });
			if (j == k) ans.push_back({ 2,1 });
		}
		else {
			ans.push_back({ 2,1 });
			if (k == u) ans.push_back({ n,n - 1 });
			else if (l == u) ans.push_back({ n - 1,n });
		}
		cout << ans.size() << '\n';
		for (pii p : ans) {
