...
void ColorTheFence() {
	...
	string ans = string(n / a[0].ff, a[0].ss + '0');
	n %= a[0].ff;
	for (int i = 0; i < ans.length(); i++) {
		int mx = ans[i] - '0', cost = 0;
		for (int j = 1; j < 9; j++) {
			if (n >= (a[j].ff - a[0].ff) and a[j].ss > a[0].ss and a[j].ss > mx) mx = a[j].ss, cost = a[j].ff;
		}
		if (mx == ans[i] - '0') break;
		else ans[i] = mx + '0', n -= (cost - a[0].ff);
	}
	cout << ans << '\n';
}