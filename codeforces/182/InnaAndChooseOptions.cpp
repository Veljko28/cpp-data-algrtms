...
int main() {
	...
	while (t--) {
		...
		cin >> s;
		vs ans;
		if (s.find('X') != string::npos) ans.push_back("1x12");
		else {
			cout << 0 << '\n';
			continue;
		}
		bool ok = 0;
		for (int i = 0; i < 6; i++) {
			ok |= (s[0 + i] == 'X' and s[6 + i] == 'X');
		}
		if (ok) ans.push_back("2x6");
		ok = 0;
		for (int i = 0; i < 4; i++) {
			ok |= (s[0 + i] == 'X' and s[4 + i] == 'X' and s[8+i] == 'X');
		}
		if (ok) ans.push_back("3x4");
		ok = 0;
		for (int i = 0; i < 3; i++) {
			ok |= (s[0 + i] == 'X' and s[3 + i] == 'X' and s[6 + i] == 'X' and s[9+i] == 'X');
		}
		if (ok) ans.push_back("4x3");
		ok = 0;
		for (int i = 0; i < 2; i++) {
			ok |= (s[0 + i] == 'X' and s[2 + i] == 'X' and s[4 + i] == 'X' and s[6 + i] == 'X' and s[8 + i] == 'X' and s[10 + i] == 'X');
		}
		if (ok) ans.push_back("6x2");
		if (count(all(s), 'X') == 12) ans.push_back("12x1");
