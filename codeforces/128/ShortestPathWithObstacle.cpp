...
void ShortestPathWithObstacle() {
	...
	while (t--) {
		...
		cin >> b.ff >> b.ss;
		cin >> f.ff >> f.ss;
		if (a.ff == b.ff and a.ff == f.ff and f.ss > min(a.ss, b.ss) and f.ss < max(a.ss, b.ss)) {
			cout << abs(a.ss - b.ss) + 2 << '\n';
		}
		else if (a.ss == b.ss and a.ss == f.ss and f.ff > min(a.ff, b.ff) and f.ff < max(a.ff, b.ff)) {
			cout << abs(a.ff - b.ff) + 2 << '\n';
		}
		else cout << abs(a.ss - b.ss) + abs(a.ff - b.ff) << '\n';
	}
}