...
int main() {
	...
	if (a.ss - b.ss != 0) ans++;
	cout << ans << ' ';
	if (abs(a.ff - b.ff) == abs(a.ss - b.ss) and a != b) {
		cout << 1 << ' ';
	}
	else {
		bool ok = 0;
		for (int i = 1; i <= 8; i++) {
			if (abs((a.ff + i) - b.ff) == abs((a.ss - i) - b.ss)) ok = 1;
			else if (abs((a.ff - i) - b.ff) == abs((a.ss + i) - b.ss)) ok = 1;
			else if (abs((a.ff - i) - b.ff) == abs((a.ss - i) - b.ss)) ok = 1;
			else if (abs((a.ff + i) - b.ff) == abs((a.ss + i) - b.ss)) ok = 1;
			if (ok) break;
		}
	...
	ans = 0;
	while (true) {
		if (!abs(a.ff - b.ff) or !abs(a.ss - b.ss)) {
