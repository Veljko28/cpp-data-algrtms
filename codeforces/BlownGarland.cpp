...
bool check(int i, char c, string a) {
	for (int j = 1; j < 4; j++) {
		if (i + j < a.length() and a[i + j] == c) return false;
		if (i - j >= 0 and a[i - j] == c) return false;
	}
	return true;
}

void recurs(int i, vector<int> clr, string a) {
	if (i == a.length()) {
		ans = clr;
		return;
	}
