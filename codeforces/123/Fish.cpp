...
vector<vector<double>> a(20, vector<double>(20, 0));
vector<double> dp((1 << 19), -1);

double pMove(int p_mask, int i, int& n) {
	int k = __builtin_popcount(p_mask);
	ll d = (k * (k - 1)) / 2;
	double prob = 0;
	for (int j = 0; j < n; j++) {
		if ((1 << j) & p_mask) {
...
double solve(int mask, int& n) {
	...
	for (int i = 0; i < n; i++) {
		if (!(mask & (1 << i))) {
			int p_mask = mask ^ (1 << i);
			double c = solve(p_mask, n);
			ans += c * pMove(p_mask, i, n);
		}
	}
	return dp[mask] = ans;
}
