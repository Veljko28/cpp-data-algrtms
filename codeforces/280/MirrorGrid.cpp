...
void solve() {
	...
	b[0] = { 0,0 }, b[1] = { 0,n + 1 }, b[2] = { n + 1,n+1 }, b[3] = { n + 1,0 };
	int ans = 0;
	for (int i = 1; i <= n; i++) {
		for (int j = 1; j <= n; j++) {
			int c = i - b[0].first, d = j - b[0].second;
			vector<pair<int, int>> f(4);
			f[0] = { i,j };
			f[1] = { b[1].first + d,b[1].second - c };
			f[2] = { b[2].first - c,b[2].second - d };
			f[3] = { b[3].first - d,b[3].second + c };
			
			int cnt0 = 0, cnt1 = 0;
			for (pair<int, int> p : f) {
				if (a[p.first][p.second] == '1') cnt1++;
				else cnt0++;
			}
			ans += min(cnt0, cnt1);
		}
