...
void SnowWalkingRobot() {
    ...
    while (q--){
        ...
        int b = min(a[0],a[1]), c = min(a[2],a[3]);
        if (b == 0 and c == 0) cout << "0" << '\n';
        else if (b >= 1 and c == 0) cout << "2" << '\n' << "LR" << '\n';
        else if (b == 0 and c >= 1) cout << "2" << '\n' << "UD" << '\n';
        else {
            string s = string(b,'R') + string(c, 'D') + string(b,'L') + string(c,'U');
            cout << (2*b)+(2*c) << '\n' << s << '\n';
        }
    }
}
