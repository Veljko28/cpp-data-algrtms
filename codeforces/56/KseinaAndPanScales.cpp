...
vector<string> split(string s, char c){
    ...
    for (int i=0;i<s.length();i++){
        if (s[i] == c) {a.pb(d); j=i; break;}
        else d += s[i];
    }
    a.pb(s.substr(j+1,s.length()-(j+1)));
    return a;
}

void KseniaAndPanScales() {
	sync;
    string s;
    cin >> s;
    ...
    cin >> t;
    while (t.length()){
        if (spt[0].length() > spt[1].length()) spt[1] += (t[0]);
        else spt[0] += (t[0]);
        t = t.substr(1,t.length());
    }
    if (spt[0].length() == spt[1].length()) outn(spt[0]+"|"+spt[1]);
    else imp;
}
